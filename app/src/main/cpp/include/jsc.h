...
#ifndef JSC_EXPORT
#define JSC_EXPORT __attribute__((visibility("default")))
#endif

namespace JSC {

    class JSC_EXPORT JSException : public std::exception {
    public:
    ...
    namespace ExceptionHandling {
        ...
        typedef ExtractedEror(*PlatformErrorExtractor)(const std::exception &ex,
                                                       const char *context);
    ...
    JSObjectRef makeFunction(JSContextRef ctx, const char *name, JSFunction function);

    JSC_EXPORT void installGlobalFunction(JSGlobalContextRef ctx, const char *name, JSFunction function);

    JSObjectRef makeFunction(JSGlobalContextRef ctx, const char *name, JSObjectCallAsFunctionCallback callback);

    JSC_EXPORT void installGlobalFunction(JSGlobalContextRef ctx, const char *name, JSObjectCallAsFunctionCallback callback);
    ...
    JSValueRef evaluateScript(JSContextRef ctx, JSStringRef script, JSStringRef sourceURL);

    #if WITH_FBJSCEXTENSIONS
        JSValueRef evaluateSourceCode(JSContextRef ctx, JSSourceCodeRef source, JSStringRef sourceURL);
    #endif
...
**/
    class JSContextLock {
        ...
        template<JSValueRef (method)(JSContextRef ctx, JSObjectRef function, JSObjectRef thisObject, size_t argumentCount, const JSValueRef arguments[], JSValueRef *exception )>
        ...
        inline JSObjectCallAsFunctionCallback exceptionWrapMethod() {
            struct funcWrapper {
                static JSValueRef call(JSContextRef ctx, JSObjectRef function, JSObjectRef thisObject, size_t argumentCount, const JSValueRef arguments[], JSValueRef *exception) {
                    try {
                        return (*method)(ctx, function, thisObject, argumentCount, arguments,
                                         exception);
                    } catch (...) {
